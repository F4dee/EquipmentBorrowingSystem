...
export default function Profile({ user }) {
    ...
    useEffect(() => {
        if (user?.id) {
            const fetchHistory = user.role === 'admin'
                ? requestApi.getAllHistory()
                : requestApi.getByUser(user.id);
            
            fetchHistory.then(res => setHistory(res.requests || [])).catch(console.error)
        }
    }, [user])
            ...
            <div className="profile-layout">
                ...
                <div className="profile-sidebar">
                    <div className="profile-card">
                        ...
                        <p className="profile-id-text">{user?.role === 'admin' ? 'Staff ID: ADM-2024-001' : 'Student ID: STU-2024-001'}</p>
                ...
                <div className="profile-main">
                    <div className="profile-stats-row">
                        <div className="stat-box">
                            ...
                            <h2 className="stat-number">{history.length}</h2>
                            <p className="stat-label-text">Total Borrows</p>
                        </div>
                        <div className="stat-box">
                            <i className="ph ph-clock stat-icon"></i>
                            <h2 className="stat-number">{history.filter(req => req.status === 'PENDING' || req.status === 'APPROVED' || req.status === 'BORROWED').length}</h2>
                            <p className="stat-label-text">Active Requests</p>
                        </div>
                        <div className="stat-box">
                            <i className="ph ph-check-circle stat-icon"></i>
                            <h2 className="stat-number">{history.filter(req => req.status === 'RETURNED').length}</h2>
                                        ...
                                        <li>
                                            ...
                                            {user?.role === 'admin' && (
                                                <span style={{ display: 'block', fontSize: '12px', color: 'var(--text-secondary)', marginTop: '4px' }}>
                                                    Borrowed by: <strong>{req.user?.name || req.user?.email || 'Unknown User'}</strong>
                                                </span>
                                            )}
                                        </li>
                                    </ul>
